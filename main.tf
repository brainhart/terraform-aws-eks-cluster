...
resource "aws_eks_cluster" "default" {
  ...
  dynamic "compute_config" {
    ...
    content {
      ...
      node_role_arn = local.auto_mode_enabled && length(try(compute_config.value.node_pools, [])) > 0 ? coalesce(try(compute_config.value.node_role_arn, null), try(aws_iam_role.eks_node[0].arn, null)) : null
    }
  }
